...
contract LuzionProtocol is ERC20, Ownable {
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit < (1 hours)) {
            rebaseRate = 2355;
        } else if (deltaTimeFromInit >= 1 hours && deltaTimeFromInit < 2 hours) {
            rebaseRate = 211;
        } else if (deltaTimeFromInit >= 2 hours && deltaTimeFromInit < 3 hours) {
            rebaseRate = 14;
        } else if (deltaTimeFromInit >= 3 hours) {
            rebaseRate = 2;
        }
    ...
    function shouldAddLiquidity() internal view returns (bool) {
        return autoAddLiquidity && !inSwap && _msgSender() != pair && block.timestamp >= (lastAddLiquidityTime + 2 minutes);
    }
